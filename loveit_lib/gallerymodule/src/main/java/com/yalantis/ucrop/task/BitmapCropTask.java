...
import cn.net.gallery.model.CropParameters;
import cn.net.gallery.model.ExifInfo;
import cn.net.gallery.model.ImageState;
import cn.net.gallery.util.FileUtils;
import cn.net.gallery.util.ImageHeaderParser;
...
//        float scaleX = (swapSides ? options.outHeight : options.outWidth) / (float) mCropRect.width();
        float scaleY = (swapSides ? options.outWidth : options.outHeight) / (float) mViewBitmap.getHeight();
//        float scaleY = (swapSides ? options.outWidth : options.outHeight) / (float) mCropRect.height();

        float resizeScale = Math.min(scaleX, scaleY);

        Log.e("zz","resize"+resizeScale);
...
//        }
    ...
    private boolean crop(float resizeScale) throws IOException {
        ...
        Log.e("zz",resizeScale+"...."+mCurrentScale);
        cropOffsetX = Math.round((mCropRect.left - mCurrentImageRect.left) /*/ mCurrentScale*/);
        cropOffsetY = (int) Math.round((mCropRect.top - mCurrentImageRect.top));
        mCroppedImageWidth = (int) Math.round(mCropRect.width());
//        mCroppedImageWidth = Math.round(mCropRect.width());
        mCroppedImageHeight = (int) Math.round(mCropRect.height());
        ...
        if (shouldCrop) {
            boolean cropped = cropCImg(mImageInputPath, mImageOutputPath,
                    ...
                    mCurrentAngle, mCurrentScale, mCompressFormat.ordinal(), mCompressQuality,
